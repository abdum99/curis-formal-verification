...
if __name__ == '__main__':
    ...
    try:
        ...
        open('chord2.dot', 'w').write(dot)
        open('chord2_output.txt', 'w').write(ivy_original_output)
        subprocess.check_call(['display', 'chord2.dot'])
        save_file = str(input('Save counter example to newfile?(y/n): ')).lower().strip()
        if save_file[0] == 'y':
            file_name = input('Name of file: ')
            file_name_dot = file_name + '.dot'
            subprocess.check_call(['cp', 'chord2.dot', file_name_dot.strip()])
            file_name_output = file_name + '_ivy_output.txt'
            subprocess.check_call(['cp', 'chord2_output.txt', file_name_output.strip()])
    except ValueError:
        print("Check output")
